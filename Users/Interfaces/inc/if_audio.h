...
#define AUDIO_BUFFER_SIZE											AUDIO_SAMPLING_FREQ * 2
/*------------------------------------------------------------------------------
...
#define DEFAULT_AUDIO_IN_FREQ               AUDIO_SAMPLING_FREQ
...
struct audio_driver {
  ...
  uint32_t  (*init)(audio_driver *, uint16_t OutputInputDevice, uint8_t Volume, uint32_t AudioFreq);
  void      (*deinit)(audio_driver * );
  uint32_t  (*read_id)(audio_driver *);
  uint32_t  (*play)(audio_driver *, uint16_t*, uint16_t);
  uint32_t  (*pause)(audio_driver *);
  uint32_t  (*resume)(audio_driver *);
  uint32_t  (*stop)(audio_driver *, uint32_t);
  uint32_t  (*set_frequency)(audio_driver *, uint32_t);
  uint32_t  (*set_volume)(audio_driver *, uint8_t);
  uint32_t  (*set_mute)(audio_driver *, uint32_t);
  uint32_t  (*set_output_mode)(audio_driver *, uint8_t);
  uint32_t  (*reset)(audio_driver *);
} ;
...
struct audio_handle {
	...
	uint16_t freq_in;
	uint16_t freq_out;
	uint16_t buffer_in[AUDIO_SAMPLING_FREQ];				//input buffer (1 second timeframe)
	uint16_t buffer_in_head;
	uint16_t buffer_in_tail;
	uint16_t buffer_out[AUDIO_SAMPLING_FREQ];				//ourput buffer (1 second timeframe)
	uint16_t buffer_out_head;
	uint16_t buffer_out_tail;
	void (* init)(audio_handle * handle);
	uint32_t (* read)(audio_handle * handle, int16_t * buffer, uint32_t size);
	void (* write)(audio_handle * handle, int16_t * buffer, uint32_t size);
};

typedef audio_driver * audio_driver_p;
typedef audio_handle * audio_handle_p;
