...
//16 byte
__align(1) typedef struct fat_Partition {
	uchar bootflag;
	uchar start_head;
	uint16 start_sector;	
	uchar type;
...
__align(1) typedef struct fat_MasterBootRecord {
	uchar code[FAT_SECTOR_SIZE];
} fat_MasterBootRecord;

__align(1) typedef struct fat_BootRecord {
	uint16 byte_per_sec;
	uchar sec_per_clus;
...
__align(1) typedef struct fat_Entry {
	uint16 media_type;
...
__align(1) typedef struct fat_DirEntry {
...
void fat_InitPartition32(fat_FileSystem * fs, fat_Partition * p, fat_BootRecord * br, void * iface);
void fat_InitPartition16(fat_FileSystem * fs, fat_Partition * p, fat_BootRecord * br, void * iface);

fat_Partition * fat_ReadPartition(uchar partition_id, void * interface);
fat_BootRecord * fat_GetBootRecord(fat_Partition * p, void * interface);
fat_FileSystem * fat_Init(uchar partition_id, void * interface);
